...
def h_of_n(M=50, A=Amp):
    ...
    print(z)
    h_array = np.zeros(50)
    for i in range(len(noise)):         # compute ith element of h
...
def apply_to_noise():
    ...
    for i in range(len(h)-1):
        new_array[i*len(noise)/len(h):(i+1)*len(noise)/len(h)]
    return new_array
...
# things to consider: The A is not the right size, no wonder it doesn't work...
# I don't get how long the h_array is supposed to be... Takes way too much computational time